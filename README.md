...
* [Lab 3 – Windows Persistence Detection (Registry Run Key)](#lab-3--windows-persistence-detection-registry-run-key)
* [Lab 4 – SIEM Threat Detection with Microsoft Sentinel](#lab-4--siem-threat-detection-with-microsoft-sentinel)
* [Lab 5 – Network Intrusion Detection using Suricata IDS](#lab-5--network-intrusion-detection-using-suricata-ids)
...
## Lab 5 – Network Intrusion Detection using Suricata IDS

### Overview

This lab demonstrates how a Network Intrusion Detection System (IDS) can detect suspicious network activity. Suricata was deployed to monitor network traffic and identify reconnaissance attempts originating from a Kali Linux system.

The objective was to simulate attacker behavior and observe how IDS signatures generate alerts that can be investigated by security analysts.
...
Suricata was configured to monitor network traffic using the Npcap interface associated with the system’s network adapter.

Command used to start Suricata:
"C:\Program Files\Suricata\suricata.exe" -c "C:\Program Files\Suricata\suricata.yaml" --pcap=\Device\NPF_{437E8C16-5A32-44DE-8EB5-0A2EFE7E244C}


Emerging Threats community rules were installed to enable signature-based intrusion detection.

### Attack Simulation

A reconnaissance scan was launched from the Kali Linux attacker machine against the Windows target system.

Example command:
nmap -sS 192.168.101.134

This scan attempts to discover open ports by sending TCP SYN packets to multiple ports on the target host.

### Detection Investigation

Suricata successfully detected suspicious network activity generated by the attacker system.

Example alert captured from Suricata logs:
ET POLICY Possible Kali Linux hostname in DHCP Request Packet

This alert indicates that the IDS detected network traffic containing a hostname associated with a penetration testing environment.

### Logs Analyzed

Suricata generates several logs that can be used during investigations:

fast.log – contains IDS alerts  
eve.json – structured event logs  
stats.log – Suricata performance statistics

### Screenshots

![Suricata Engine Running](screenshots/lab5/suricata-engine-running.png)

![Attack Simulation from Kali](screenshots/lab5/nmap-portscan-attack.png)

![IDS Alert Detection](screenshots/lab5/suricata-alert-detection.png)

![Network Traffic Analysis](screenshots/lab5/suricata-kali-traffic-analysis.png)
...
Port scanning is commonly used by attackers to identify open services before attempting exploitation.
